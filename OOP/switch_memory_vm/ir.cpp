...
int generate(NodePtr node, std::vector<Instruction>& code) {
    ...
    case NodeKind::WHILE: {
        ...
        auto* prev = g_breakPatches; g_breakPatches = &breakPatches;
        ...
        for (int idx : breakPatches)
            code[idx].offset = relOffset(idx, (int)code.size());
    ...
    case NodeKind::SWITCH: {
        ...
        for (auto& caseNode : node->stmts) {
            ...
            Instruction jmp; jmp.op = OpCode::JMP; jmp.rd = (int)code.size();
            code.push_back(jmp); breakPatches.push_back(jmp.rd);

            code[brIdx].offset = relOffset(brIdx, (int)code.size());
        }

        int endIdx = (int)code.size();
        for (int idx : breakPatches)
            code[idx].offset = relOffset(idx, endIdx);
