...
static int relOffset(int from, int to) {
    return to - from - 1;
}
...
static OpCode negatedBranch(const std::string& op) {
    ...
    throw std::runtime_error("negatedBranch: unknown op: " + op);
}
...
static OpCode cmpOpcode(const std::string& op) {
    ...
    throw std::runtime_error("cmpOpcode: unknown op: " + op);
}
...
double execute(std::vector<Instruction>& code,
    ...
    while (pc < N) {
        ...
        case OpCode::LOAD_CONST:
            mem[ins.rd] = ins.value;
            break;

        case OpCode::LOAD_VAR:
            mem[ins.rd] = vars.count(ins.varName) ? vars[ins.varName] : 0.0;
            break;

        case OpCode::STORE_VAR:
            vars[ins.varName] = mem[ins.rs1];
            mem[ins.rd]       = mem[ins.rs1];
            break;
        ...
        case OpCode::BR_GT:
            if (mem[ins.rs1] >  mem[ins.rs2]) { pc = pc + 1 + ins.offset; continue; }
            break;
        case OpCode::BR_LT:
            if (mem[ins.rs1] <  mem[ins.rs2]) { pc = pc + 1 + ins.offset; continue; }
            break;
