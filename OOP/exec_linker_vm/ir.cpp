...
const char* opName(OpCode op) {
    switch (op) {
        ...
        case OpCode::BRL:        return "BRL";
        case OpCode::MOV_R:      return "MOV_R";
        case OpCode::MOV_IP_R:   return "MOV_IP_R";
...
std::map<std::string,int>& getFuncAddrs() { return g_funcAddr; }
...
double execute(std::vector<Instruction>& code, std::map<std::string,double>& vars) {
    ...
    while (pc >= 0 && pc < N) {
        ...
        // BRL: branch if rs1 < rs2, store return address in rd
        // Rt = IP; IP = IP + Rs  (whiteboard)
        case OpCode::BRL:
            if (reg[ins.rs1] < reg[ins.rs2]) {
                reg[ins.rd] = pc + 1;
                pc = pc + 1 + ins.offset; continue;
            } break;
        // MOV_R: Rd = offset (load immediate offset into register)
        case OpCode::MOV_R:
            reg[ins.rd] = ins.offset; break;
        // MOV_IP_R: IP = IP + Rs  (indirect jump via register)
        case OpCode::MOV_IP_R:
            pc = pc + 1 + (int)reg[ins.rs1]; continue;
