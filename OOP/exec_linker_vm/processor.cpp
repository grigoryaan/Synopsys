...
void Processor::fetch() { IR = mem.readInstruction(IP); }
void Processor::decode() { jumped = false; ++IP; }
...
void Processor::execute() {
    ...
    case OpCode::JA: {
        uint32_t a=(uint32_t)(int32_t)freg[IR.rs1], b=(uint32_t)(int32_t)freg[IR.rs2];
        if (a > b) { IP = IP + IR.offset; jumped = true; } break;
    }
    case OpCode::BRL:
        if (freg[IR.rs1] < freg[IR.rs2]) {
            freg[IR.rd] = IP;
            IP = IP + IR.offset; jumped = true;
        } break;
    case OpCode::MOV_R:    freg[IR.rd] = IR.offset; break;
    case OpCode::MOV_IP_R: IP = IP + (int)freg[IR.rs1]; jumped = true; break;
    ...
    case OpCode::LOAD_BP: {
        ...
        if (!IR.varName.empty()) vars[IR.varName] = freg[IR.rd];
        break;
    }
